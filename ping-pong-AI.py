...
class player:
    ...
    def distance_between_ball_and_enemy(self, Ball):
        distance = math.sqrt((math.pow((self.posX + self.width) - (Ball.posX - Ball.radius), 2)) + (math.pow((self.posY + self.height // 2) - Ball.posY, 2)))
        if distance <= 450:
            return True
...
class CollisionManager:
    def collision01(self, Ball, Player01):
        if Ball.posY + Ball.radius >= Player01.posY and Ball.posY - Ball.radius <= Player01.posY + Player01.height:
            if Ball.posX - Ball.radius == Player01.posX + Player01.width:
                return True
        return False

    def collision02(self, Ball, Player02):
        if Ball.posY + Ball.radius >= Player02.posY and Ball.posY - Ball.radius <= Player02.posY + Player02.height:
...
class rival_AI:
    def top(self, Ball, Player):
        if Ball.posY - Ball.radius >= 0 and Ball.posY + Ball.radius <= Player.posY:
            return True

        return False

    def bottom(self, Ball, Player):
        if Ball.posY - Ball.radius >= Player.posY + Player.height and Ball.posY + Ball.radius <= 400:
            return True
...
while is_true:
    ...
    if Player01.distance_between_ball_and_enemy(Ball):
        if Rival.top(Ball, Player01):
            Player01.update()
            Player01.state = 'up'
        if Rival.bottom(Ball, Player01):
            Player01.update()
            Player01.state = 'down'
    ...
    if playing == True:
        ...
        if win.check_win(Ball):
            fix = False
            Player01.posY = (height // 2) - 60
            Player02.posY = (height // 2) - 60
            Ball.return_to_center()
